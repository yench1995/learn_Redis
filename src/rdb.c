...
/*
 * 读入一个被编码的长度值
 * 如果length值不是整数，而是一个被编码后值，那么isencoded将被设为1
 * length的编码长度可能是1字节，2字节和5字节，分别对应6bit, 14bit 和 32bit
 */
...
/*
 * 尝试使用特殊的编码来保存value，这要求它的值必须在给定范围之内
 * 如果可以编码的话，将编码后的值保存在enc指针中，并返回编码后的长度
 */
...
/*
 * 载入被编码成指定类型的编码整数对象
 * 如果encode参数被设置了的话，那么可能会返回一个整数编码的字符串对象
 * 否则，字符串总是未编码的
 */
...
//尝试将字符串编码成整数，成功的话返回保存整数值所需要的字节数
int rdbTryIntegerEncoding(char *s, size_t len, unsigned char *enc) {
    long long value;
    char *endptr, buf[32];

    // 尝试将值转换为整数
    //entptr返回第一个不能被转换成整数的值
